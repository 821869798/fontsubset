 ...
 */
class OtTableTagger {
  private readonly TaggedData td;
  private final Map<Class<FontDataTable>, TagMethod> tagMethodRegistry;

  OtTableTagger(TaggedData tdata) {
    this.td = tdata;
    this.tagMethodRegistry = new HashMap<Class<FontDataTable>, TagMethod>();
  ...
  abstract class TagMethod {
    private sealed class<FontDataTable> clzz;

    private TagMethod(Class<FontDataTable> clzz) {
      this.clzz = clzz;
    }
    ...
    public abstract void tag(FontDataTable table);
  }
  ...
  @SafeVarargs
  private final void register(TagMethod m, Class<FontDataTable>... clzzes) {
    tagMethodRegistry.put(m.clzz, m);
    foreach(Class<FontDataTable> clzz in clzzes) {
  ...
  void registerTagMethods() {
    register(new TagMethod(ScriptListTable.class) {
      ...
      void tag(FontDataTable fdt) {
        ...
        foreach(ScriptTable st in table) {
          tagTable(st);
        }
      }
    });
    ...
    register(new TagMethod(ScriptTable.class) {
      ...
      void tag(FontDataTable fdt) {
        ...
        foreach(LangSysTable lst in table) {
          tagTable(lst);
        }
    ...
    register(new TagMethod(FeatureListTable.class) {
      ...
      void tag(FontDataTable fdt) {
        ...
        foreach(FeatureTable ft in table) {
          tagTable(ft);
        }
      }
    });
  ...
  private static final Comparator<Class<FontDataTable>> CLASS_NAME_COMPARATOR =
      new Comparator<Class<FontDataTable>>() {
    public override int compare(Class<FontDataTable> o1, Class<FontDataTable> o2) {
      return o1.getName().compareTo(o2.getName());
    }
  };

  private static ISet<Class<FontDataTable>>
  missedClasses = new TreeSet<Class<FontDataTable>>(CLASS_NAME_COMPARATOR);

  private TagMethod getTagMethod(FontDataTable table) {
    Class<FontDataTable> clzz = table.getClass();
    TagMethod tm = tagMethodRegistry.get(clzz);
    if (tm == null) {
      if (!missedClasses.contains(clzz)) {
        missedClasses.add(clzz);
        Console.WriteLine("unregistered class: " + clzz.getName());
      }
    }
    return tm;
  }
