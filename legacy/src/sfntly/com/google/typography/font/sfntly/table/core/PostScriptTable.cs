 ...
 * @author Stuart Gill
 */
public sealed class PostScriptTable : Table
{

    private static readonly int VERSION_1 = 0x10000;
    ...
    private AtomicReference<IList<String>> names = new AtomicReference<IList<String>>();
    ...
    private PostScriptTable(Header header, ReadableFontData data) : base(header, data)
    {
    }
    ...
    {
        return this._data.readFixed((int)Offset.italicAngle);
    }

    public int underlinePosition()
    {
        return this._data.readFWord((int)Offset.underlinePosition);
    }

    public long isFixedPitchRaw()
    {
        return this._data.readULong((int)Offset.isFixedPitch);
    }
    ...
    {
        return this._data.readULong((int)Offset.minMemType42);
    }

    public long maxMemType42()
    {
        return this._data.readULong((int)Offset.maxMemType42);
    }

    public long minMemType1()
    {
        return this._data.readULong((int)Offset.minMemType1);
    }

    public long maxMemType1()
    {
        return this._data.readULong((int)Offset.maxMemType1);
    }
    ...
    public int numberOfGlyphs()
    {
        ...
        {
            return this._data.readUShort((int)Offset.numberOfGlyphs);
        }
        else
        {
    ...
    public String glyphName(int glyphNum)
    {
        ...
        {
            glyphNameIndex = this._data.readUShort((int)Offset.glyphNameIndex + 2 * glyphNum);
        }
        else
        {
            return null;
    ...
    private IList<String> parse()
    {
        ...
        if (version() == VERSION_2)
        {
            ...
            int index = (int)Offset.glyphNameIndex + 2 * numberOfGlyphs();
            while (index < dataLength())
            {
                int strLen = this._data.readUByte(index);
                byte[] nameBytes = new byte[strLen];
                this._data.readBytes(index + 1, nameBytes, 0, strLen);
                try
                {
                    names.Add(Encoding.GetEncoding("ISO-8859-1").GetString(nameBytes)  /*new String(nameBytes, "ISO-8859-1")*/);
                }
    ...
    public interface IBuilder: ITableBasedTableBuilder<PostScriptTable>
    {
        
    }

    private class Builder : TableBasedTableBuilder<PostScriptTable>, IBuilder
    {
        ...
        public override PostScriptTable subBuildTable(ReadableFontData data)
        {
