...
public class FontInfo {
  private static readonly int CHECKSUM_LENGTH = 8;
  private static readonly DecimalFormat twoDecimalPlaces = new DecimalFormat("#.##");
  ...
  public static DataDisplayTable listTables(Font font) {
    ...
    // Calculate font size
    IEnumerator<Table> fontTableIter = font.GetEnumerator();
    while (fontTableIter.MoveNext()) {
      Table fontTable = fontTableIter.Current;
      fontSize += fontTable.headerLength();
    }

    // Add table data to output string
    fontTableIter = font.GetEnumerator();
    while (fontTableIter.MoveNext()) {
      Table fontTable = fontTableIter.Current;
  ...
  public static DataDisplayTable listNameEntries(Font font) {
    ...
    foreach(NameEntry entry in nameTable) {
  ...
  public static DataDisplayTable listCharBlockCoverage(Font font) {
    ...
    foreach(String charStr in controlSet) {
      if (cmap.glyphId(UCharacter.codePointAt(charStr, 0)) > 0) {
        totalCount++;
      }
    }
  ...
  public static DataDisplayTable listScriptCoverage(Font font) {
    ...
    DataDisplayTable table = new DataDisplayTable(Arrays.asList(header));
    table.setAlignment(Arrays.asList(displayAlignment));
    HashIDictionary<Integer, Integer> coveredScripts = new HashIDictionary<Integer, Integer>();
    ...
    // coverage to table
    ISet<Integer> sortedScripts = new TreeSet<Integer>(coveredScripts.keySet());
    int unknown = 0;
    foreach(Integer scriptCode in sortedScripts) {
  ...
  public static DataDisplayTable listCharsNeededToCoverScript(Font font) {
    ...
    DataDisplayTable table = new DataDisplayTable(Arrays.asList(header));
    table.setAlignment(Arrays.asList(displayAlignment));
    HashIDictionary<Integer, UnicodeSet> coveredScripts = new HashIDictionary<Integer, UnicodeSet>();
    ...
    foreach(Integer scriptCode in sortedScripts) {
      ...
      foreach(String charStr in uSet) {
  ...
  public static DataDisplayTable listSubglyphFrequency(Font font) {
    IDictionary<Integer, Integer> subglyphFreq = new HashIDictionary<Integer, Integer>();
    ...
    int numSubglyphs = 0;
    ISet<Integer> sortedKeySet = new TreeSet<Integer>(subglyphFreq.keySet());
    foreach(Integer key in sortedKeySet) {
  ...
  public static DataDisplayTable listUnmappedGlyphs(Font font) {
    ...
    ISet<Integer> mappedGlyphs = new HashSet<Integer>();
    CMapTable cmapTable = FontUtils.getCMapTable(font);
    foreach(CMap cmap in cmapTable) {
      foreach(Integer codePoint in cmap) {
