 ...
 */
public class ReadableFontData : FontData {
   ...
   */
  public ReadableFontData(ByteArray array) : base(array)
	{
  }
   ...
   */
  public ReadableFontData(ReadableFontData data, int offset):base(data,offset) {
  }
   ...
   */
  public ReadableFontData(ReadableFontData data, int offset, int length):base(data, offset, length)
	{
  }
   ...
   */
  new public ReadableFontData slice(int offset, int length) {
    if (offset < 0 || length < 0 || offset > Integer.MaxValue - length ||
   ...
   * @return a slice of the original FontData
   */
  new public ReadableFontData slice(int offset) {
    ...
    protected override FontData InternalSlice(int offset)
    {
        return slice(offset);
    }

    protected override FontData InternalSlice(int offset, int length)
    {
   ...
   * calculation. Updates the public state of this object in a threadsafe way.
   */
  private void computeChecksum() {
    lock (this.checksumLock) {
      if (this.checksumSet) {
      ...
      this._checksum = sum & 0xffffffffL;
   ...
   * calculation.
   *
   * <p>Only called from within a lock method so it does not need to be
   * lock itself.
   *
   ...
   */
  public void setCheckSumRanges(params int[] ranges) {
    lock (this.checksumLock) {
  ...
  public int readUByte(int index) {
    if (!this.boundsCheck(index, 1)) {
      throw new IndexOutOfBoundsException(
          "Index attempted to be read from is out of bounds: " + NumberHelper.toHexString(index));
    }
   ...
   * @param start(int)Offset.the between subsequent start values
   * @param endIndex the position to read the first end value from
   * @param end(int)Offset.the between subsequent end values
