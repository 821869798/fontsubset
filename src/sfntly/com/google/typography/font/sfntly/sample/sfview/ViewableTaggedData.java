...
class ViewableTaggedData {
  ...
  ViewableTaggedData(List<Marker> markers) :this(markers, new Style(), new Metrics()) {
  }
  ...
  public class TaggedDataImpl : TaggedData {
    private final List<Marker> markers = new ArrayList<Marker>();
    private RangeNode rangeStack;
    private readonly PostScriptTable post;
    ...
    public override void tagField(int position, int width, int value, String alt, String label) {
      markers.add(new Field(position, width, value, alt, label));
    }

    public override void tagTarget(int position, int value, int targetPosition, String label) {
    ...
    public override void pushRange(String label, ReadableFontData data) {
      rangeStack = new RangeNode(label, data, rangeStack, data.dataOffset());
    }

    public override void pushRangeAtOffset(String label, int base) {
      if (rangeStack == null) {
    ...
    public override void setRangePosition(int rangePosition) {
      if (rangeStack == null) {
        throw new IllegalStateException("not in a range");
      }
    ...
    public override int tagRangeField(FieldType ft, String label) {
      if (rangeStack == null) {
        throw new IllegalStateException("not in a range");
      }
  ...
  private static class DrawContext {
    private readonly Style style;
    private readonly Metrics metrics;
    private readonly Graphics g; // if null, we are measuring
    private FontRenderContext frc; // used when measuring
    private readonly int x; // current position of 'position' column (margin is to
    // left)
  ...
  private static class WidthUsageRecord {
    ...
    private int width;
    private int src;
    static private readonly WidthUsageRecord EMPTY = new WidthUsageRecord();
  ...
  private static class RefWidthFinder {
    ...
    static private readonly int MAX_WIDTH = 600;
  ...
  private static abstract class Marker : Comparable<Marker> {
    ...
    public override int compareTo(Marker rhs) {
      int result = this.position - rhs.position;
      if (result != 0) {
        return result;
      }
      Class<Marker> thisClass = this.getClass();
      Class<Marker> thatClass = rhs.getClass();
    ...
    private static int classOrder(Class<Marker> clzz) {
  ...
  private static class RangeStart : Marker {
    private readonly Range range;

    private RangeStart(Range range) :base(range.start()) {
      this.range = range;
    }
  ...
  private static class RangeEnd : Marker {
    private readonly Range range;

    private RangeEnd(Range range) :base(range.limit()) {
      this.range = range;
    }
  ...
  private static class Field : Marker {
    private readonly int width;
    private readonly int value;
    private readonly String alt;
    private readonly String label;

    private Field(int position, int width, int value, String alt, String label) :base(position) {
      this.width = width;
      this.value = value;
      this.alt = alt;
      this.label = label;
  ...
  private static class ReferenceSource : Marker {
    private readonly Reference ref;
    private readonly int value;
    private readonly String label;

    private ReferenceSource(Reference ref, int value, String label) :base(ref.sourcePosition) {
      this.ref = ref;
      this.value = value;
      this.label = label;
    }
  ...
  private static class ReferenceTarget : Marker {
    private readonly Reference ref;

    private ReferenceTarget(Reference ref) :base(ref.targetPosition) {
      this.ref = ref;
    }
