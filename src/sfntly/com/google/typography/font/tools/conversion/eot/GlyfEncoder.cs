...
public class GlyfEncoder {
  ...
  private readonly boolean doPush;
  
  private readonly ByteArrayOutputStream glyfStream;
  private readonly ByteArrayOutputStream pushStream;
  private readonly ByteArrayOutputStream codeStream;
  ...
  public GlyfEncoder() :this(true) {
  }

  public void encode(Font sourceFont) {
    LocaTable loca = sourceFont.getTable<LocaTable>(Tag.loca);
    int nGlyphs = loca.numGlyphs();
    GlyphTable glyf = sourceFont.getTable<GlyphTable>(Tag.glyf);
  ...
  private void writeGlyph(Glyph glyph) {
    try {
      if (glyph == null || glyph.dataLength() == 0) {
        writeUShort(0);
      } else if (glyph is SimpleGlyph) {
        writeSimpleGlyph((SimpleGlyph)glyph);
      } else if (glyph is CompositeGlyph) {
  ...
  private void writeInstructions(Glyph glyph) {
    if (doPush) {
      splitPush(glyph);
    } else {
      int pushCount = 0;
  ...
  private void writeSimpleGlyph(SimpleGlyph glyph)  {
  ...
  private void writeCompositeGlyph(CompositeGlyph glyph)  {
  ...
  // visible for testing
  static void write255UShort(OutputStream os, int value)  {
    if (value < 0) {
  ...
  // As per 6.1.1 of spec
  // visible for testing
  static void write255Short(OutputStream os, int value)  {
  ...
  // visible for testing
  void writeTriplet(OutputStream os, boolean onCurve, int x, int y)  {
    int absX = Math.Abs(x);
    int absY = Math.Abs(y);
  ...
  private void encodePushSequence(OutputStream os, IList<Integer> data)  {
    int n = data.Count;
    int hopSkip = 0;
    for (int i = 0; i < n; i++) {
      if ((hopSkip & 1) == 0) {
